...
public class CenterServiceTest {
    ...
    void 보호소_상세조회를_할수있다(){
        ...
        FindCenterByIdResponse center1 = FindCenterByIdResponse.builder()
                .message("보호소 조회 성공")
                .status("200")
                .uuid("aa1")
                .name("name")
        ...
        //when
        FindCenterByIdResponse findCenter = centerService.findOne("aa1");

        //then
        then(centerRepository).should().findById("aa1");

        assertThat(findCenter).isEqualTo(center1);
